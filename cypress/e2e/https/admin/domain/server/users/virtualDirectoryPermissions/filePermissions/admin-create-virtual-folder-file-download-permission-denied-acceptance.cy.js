...
/**
 * @description
 * This spec file contains test to verify that user cannot download a file when its permission is not granted
 *
 * @breadcrumb
 * Login > {existing server} > users > create new virtual folder
 *
 * @assertions
 * To verify that user cannot download a file when its permission is not granted
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const createUserDetails = {
    username: `qa-auto-deny-file-download-user-${Cypress.dayjs().format('ssmmhhMMYY')}`,
    password: '123456',
  ...
  const localPathForDownload = '../fixtures/local.txt'
  ...
  it('verify that user cannot download a file when its permission is not granted', () => {
    ...
    cy.task('sftpDownLoadFile', { newRemoteDir: newDir, localPathForDownload, configSFTP }).then(p => {
      expect(`${JSON.stringify(p)}`).to.equal(`"fastGet->sftp: I/O error. Remote: ${newDir} Local: ${localPathForDownload}"`)
      cy.task('endSFTPConnection')
    })
    // deleting the file
