...
/**
 * @description
 * This spec file contains test to verify that admin can create an event before user uploads a file, custom file with custom text is also created
 *
 * @breadcrumb
 * Login > {existing server} > events > create new event
 *
 * @assertions
 * To verify that admin can create an event before user uploads a file
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const createUserDetails = {
    username: `qa-auto-before-file-upload-event-user-${Cypress.dayjs().format('ssmmhhMMYY')}`,
    password: '123456',
  ...
  const eventName = `file_upload_event${Cypress.dayjs().format('ssmmhhMMYY')}`
  const eventDescription = 'this event is used to write to a file when a file upload starts'
  const customFileName = 'log.txt'
  const customFilePath = './log.txt'
  const customText = `this file ${customFileName} was created before ${remoteDirFile} was uploaded`
  ...
  it('creating new event for file upload/write', () => {
    ...
    // adding event
    cy.createEvent(label.fileEvents, label.fileUploadWrite, label.beforeUpload)
