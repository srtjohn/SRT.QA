...
/**
 * @description
 * This spec file contains test to verify that admin can create an event when user download a file, custom file with custom text is also created
 *
 * @breadcrumb
 * Login > {existing server} > events > create new event
 *
 * @assertions
 * To verify that admin can create an event when user download a file
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const createUserDetails = {
    username: `qa-auto-file-download-event-user-${Cypress.dayjs().format('ssmmhhMMYY')}`,
    password: '123456',
  ...
  const eventDescription = 'this event is used to write to a file when a file download happens'
  const customFileName = 'log.txt'
  const customFilePath = './log.txt'
  const customText = `this file ${customFileName} was created when ${remoteDirFile} was downloaded`
  ...
  it('creating new event for file download/read', () => {
    ...
    cy.createEvent(label.fileEvents, label.fileDownloadRead, label.downloadReadSuccess)
