...
/**
 * @description
 * This spec file contains test to verify that admin can create an event before user append a file, custom file with custom text is also created
 *
 * @breadcrumb
 * Login > {existing server} > events > create new event
 *
 * @assertions
 * To verify that admin can create an event before user append a file
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const createUserDetails = {
    username: `qa-auto-before-file-append-event-user-${Cypress.dayjs().format('ssmmhhMMYY')}`,
    password: '123456',
  ...
  const eventDescription = 'this event is used to write to a file before a file append happens'
  const customFileName = 'log.txt'
  const customFilePath = './log.txt'
  const customText = `this file ${customFileName} was created before ${remoteDirFile} was appended`
  ...
  it('creating new event for file append', () => {
    ...
    cy.createEvent(label.fileEvents, label.fileAppend, label.beforeAppend)

    // adding action
    cy.createAction(actionType, customText, filePath, eventName, eventDescription)

    // verify that log file is not created before trigger
