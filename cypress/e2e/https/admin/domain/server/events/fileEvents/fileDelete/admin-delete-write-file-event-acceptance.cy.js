...
/**
 * @description
 * This spec file contains test to verify that admin can create an event when user delete a file, custom file with custom text is also created
 *
 * @breadcrumb
 * Login > {existing server} > events > create new event
 *
 * @assertions
 * To verify that admin can create an event when user delete a file
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const customText = `this file ${customFileName} was created when ${remoteDirFile} was deleted`
  ...
  it('creating new event for file delete', () => {
    ...
    cy.createEvent(label.fileEvents, label.fileDelete, label.deleteSuccess)
