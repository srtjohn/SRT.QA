...
/**
 * @description
 * This spec file contains test to verify that admin can create an event before user download a file, custom file with custom text is also created
 *
 * @breadcrumb
 * Login > {existing server} > events > create new event
 *
 * @assertions
 * To verify that admin can create an event before user download a file
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const createUserDetails = {
    username: `qa-auto-before-file-download-event-user-${Cypress.dayjs().format('ssmmhhMMYY')}`,
    password: '123456',
  ...
  const localPathForDownload = './../fixtures/local2.txt'
  const remoteDir = '/'
  const localPath = '../fixtures/local.txt'
  const remoteDirFile = './file2.txt'
  const actionType = label.writeToFile
  const eventName = `file_download_event${Cypress.dayjs().format('ssmmhhMMYY')}`
  const eventDescription = 'this event is used to write to a file before a file download starts'
  const customFileName = 'log.txt'
  const customFilePath = './log.txt'
  const customText = `this file ${customFileName} was created before ${remoteDirFile} was downloaded`
  ...
  it('creating new event for file download/read', () => {
    ...
    cy.createEvent(label.fileEvents, label.fileDownloadRead, label.beforeDownload)

    // adding action
    cy.createAction(actionType, customText, filePath, eventName, eventDescription)

    cy.task('sftpDownLoadFile', { newRemoteDir: remoteDirFile, localPathForDownload, configSFTP }).then(p => {
      expect(`${JSON.stringify(p)}`).to.equal(`"${remoteDirFile} was successfully download to ${localPathForDownload}!"`)
      cy.task('endSFTPConnection')
    })
