...
/**
 * @description
 * This spec file contains test to verify that admin can create an event before user rename a file, custom file with custom text is also created
 *
 * @breadcrumb
 * Login > {existing server} > events > create new event
 *
 * @assertions
 * To verify that admin can create an event before user rename a file
 *
 *  @prerequisites
 * Pre-Requisite data:
...
describe('Login > {existing server} > events > create new event', () => {
  ...
  const createUserDetails = {
    username: `qa-auto-before-file-rename-event-user-${Cypress.dayjs().format('ssmmhhMMYY')}`,
    password: '123456',
  ...
  const eventName = `before_file_rename_event${Cypress.dayjs().format('ssmmhhMMYY')}`
  const eventDescription = 'this event is used to write to a file when a file rename starts'
  const customFileName = 'log.txt'
  const customFilePath = './log.txt'
  const customText = `this file ${customFileName} was created before ${remoteDirFile} was renamed`
  ...
  it('creating new event for file rename', () => {
    ...
    cy.createEvent(label.fileEvents, label.fileRename, label.beforeRename)
