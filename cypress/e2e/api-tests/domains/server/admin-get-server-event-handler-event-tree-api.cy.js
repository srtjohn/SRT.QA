/**
 * @description
 * This spec file contains test to ensure admin can get server event handler event tree through API
 *
 * @assertions
 * To verify that admin can get server event handler event tree through API
 *
 *  @prerequisites
...
describe('GET /api/Servers', () => {
  ...
  const RequestType = 'EventHandlerEventTree'
  ...
  it('verify that admin can get event handler event tree through API', () => {
    cy.getServerEventsApiRequest(serverDetails, RequestType).then(($response) => {
      // Check if response type is Api Event Handler Event Tree
      expect($response.ResponseType).to.equal('ApiEventHandlerEventTree')
      // Check if ErrorStr is equal to success
      expect($response.Result.ErrorStr).to.equal('Success')
      // check if created event is present in response or not
      expect($response.Response.Name).to.equal('All Events')
